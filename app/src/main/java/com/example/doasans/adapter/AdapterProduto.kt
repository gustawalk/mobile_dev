...
class AdapterProduto(private val context: Context, private val produtos: MutableList<Produto>): RecyclerView.Adapter<AdapterProduto.ProdutoViewHolder>(){
    ...
    override fun onBindViewHolder(holder: ProdutoViewHolder, position: Int) {
        ...
        // Acesse o botão Resgatar no ViewHolder
        val resgatarButton = holder.itemView.findViewById<Button>(R.id.resgatarButton)

        // Configure o clique do botão
        resgatarButton.setOnClickListener {
            Toast.makeText(context, "Resgatado com sucesso! As informações serão enviadas por email.", Toast.LENGTH_LONG).show()
        }
    }
